...
echo "hello.";
  ...
  $defaults = [
        'flashairip' => '192.168.8.2',
        'syncfrom' => '/DCIM/100__TSB',
        'syncto' => getenv('HOME') . '/img',
        'timezone' => 'GMT',
        'pingtimeout' => '5'
  ];
  ...
  $options = array();
	...
	$FlashAirIP  = $options['192.168.3.2']; // IP address of the flashair card, if the card is running in the default host mode you could just use the hostname ("flashair")
	$SyncFrom    = $options['/DCIM/100__TSB'];         // Path on the sdcard to copy from (recursive)
	$SyncTo      = $options['/img']; // Full path where it will copy to (recursive)
	$TZ          = $options['GMT'];          // Camera Timezone
